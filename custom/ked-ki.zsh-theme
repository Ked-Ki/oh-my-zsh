...
function git_prompt_info_override() {
  local ref
  if [[ "$(command git config --get oh-my-zsh.hide-status 2>/dev/null)" != "1" ]]; then
    ref="${$(command git symbolic-ref HEAD 2> /dev/null)#refs/heads/}" || \
    ref="$yellow$(command git describe --tags --exact-match  2> /dev/null)" || \
    ref="$cyan$(command git rev-parse --short HEAD 2> /dev/null)" || return 0
    echo "$ZSH_THEME_GIT_PROMPT_PREFIX${ref}$(parse_git_dirty)$ZSH_THEME_GIT_PROMPT_SUFFIX"
  fi
}

PROMPT='$username_output$hostname_output:$current_dir_output%1(j. [$jobs_bg].)'
GIT_PROMPT='$(out=$(git_prompt_info_override)$(git_prompt_status)$(git_remote_status);if [[ -n $out ]]; then printf %s " $white($green$out$white)$reset";fi)'
PROMPT+="$GIT_PROMPT"
